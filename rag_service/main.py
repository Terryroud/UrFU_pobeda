...
class RAG:
    ...
    def create_faiss_index(self):
        ...
        send_log("INFO", "Faiss create successful")
    ...
    def rag_request(self, question):
        ...
        send_log("INFO", f"RAG found {len(retrieved_docs)} suitable chunks. RAG answer: {context_chunks}")
...
async def create_index():
    try:
        rag_instance.create_faiss_index()
        return {"status": "index created"}
    except Exception as e:
        send_log("ERROR", f"Error creating index: {str(e)}")
        return {"error": str(e)}

@app.post("/query")
async def query_rag(request: QueryRequest):
    try:
        context = rag_instance.rag_request(request.question)
        return {"context": context}
    except Exception as e:
        send_log("ERROR", f"Error querying RAG: {str(e)}")
        return {"error": str(e)}
